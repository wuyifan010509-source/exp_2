...
COST_CONFIG = {
    ...
    "error_cost": {
        "low": 1,      # 低风险问题答错损失
        "mid": 10,     # 中风险问题答错损失  
        "high": 1000,  # 高风险问题答错损失（提高到2000元）
    },
    # 延迟代价（客户流失成本，每毫秒）
    # 假设等待30秒导致客户不满意，成本约等于mid错误
    "latency_cost_per_ms": 0.0003,  # 每秒1元 = 每毫秒0.001元
    # 人工固定成本（每次介入）
    "human_fixed_cost": 50  # 每次人工服务成本
}

# 排队模型参数（实际场景：5个专家，2分钟/单）
QUEUE_CONFIG = {
    "service_rate_mu": 1/120,  # 人工服务率：每秒处理 1/120 个请求（平均120秒=2分钟/单）
    "num_servers_c": 5,        # 5个专家
    "max_queue_length": 10000,   # 最大队列长度（设为很大值，避免策略被队列限制干扰）
}
